...
alignmentthresholdplot<-function(){
  ...
  if (tolower(condit.input)=="y"){
    ...
    for (i in 1:Threshold.max){
      ...
      ## set shape
      if (length(unique(na.omit(eg2.l$Invariant))) == 2){
        group.shape <- c(17,16)
      } else if (unique(na.omit(eg2.l$Invariant)) == "True"){
        group.shape <- 16
      } else {group.shape <- 17}

      ## Plotting ggplot
      gg <- ggplot(eg2.l, aes(x = fct_reorder(Item, eg2.l[,4],na.rm = T,.desc=T), y = eg2.l[,4])) +
...
alignmentloadingplot<-function(){
  ...
  if (tolower(condit.input)=="y"){
    ...
    ## Plotting gg
    gg <- ggplot(eg2.l, aes(x = fct_reorder(Item, eg2.l[,4]), y = eg2.l[,4])) +
      geom_point(aes(shape = Invariant, colour = lclass), size = 3) +
      scale_shape_manual(values = group.shape, na.translate = F) +
      scale_colour_manual(name = "Group\n(Latent Class&\nInvariant)", values = group.colors, labels = GroupLabel) +
      coord_flip() +
