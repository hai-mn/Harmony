...
convert2irt <- function(){
  ...
  loadings.file <- read.csv(file = paste0(filepath,"/loadings.csv"))
  ...
  write.csv(loadings.file, paste0(filepath,"/discriminations.csv"), row.names=FALSE)
  ...
  for (h in 1:Threshold.max){
    thresholds.file[[h]] <- read.csv(file = paste0(filepath,"/threshold",h,".csv"))
    ...
    write.csv(thresholds.file[[h]], paste0(filepath,"/difficulty",h,".csv"), row.names=FALSE)
  }
}
