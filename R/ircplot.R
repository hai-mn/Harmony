...
#' @export irc
#' @import tidyverse latex2exp ggpubr
...
irc <- function(){
  ...
  discriminations.file <- read.csv(file = paste0(filepath,"/discriminations.csv"))
  ...
  for (h in 1:Threshold.max){
    difficulty.file[[h]] <- read.csv(file = paste0(filepath,"/difficulty",h,".csv"))
    ...
    total <- full_join(total, difficulty.file[[h]], by = "Item")

  }

  # output the total file including item, discriminations and difficulties for reference
  write.csv(total, paste0(filepath,"/irc_file.csv"), row.names=FALSE)
  ...
  #selected.item <- toupper(readline(prompt="Input an item need to be plotted: "))

  #cat("- The Number of Groups (Latent Classes):", Group,"\n")
  #selected.Group <<- as.numeric(readline(prompt="Input the Group or leave blank if you want to plot all groups: "))
  ...
  if (selected.group != (Group+1)){
    ...
    i <- selected.group
      ...
      item[[i]] <- total %>% filter(Item == selected.item) %>%
        select(., which(names(.) %in% keep.var))
    ...
    figure <- ggarrange(irc, circ, ncol = 1, nrow = 2)
